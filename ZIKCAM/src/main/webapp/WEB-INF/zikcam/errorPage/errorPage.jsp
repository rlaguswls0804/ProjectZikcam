...
<body>
	...
	<br/>
	<b>1. 로그인된 아이디를 확인해주세요.</b>
	<ul>
		<li>2. 권한이 없으면 접근할 수 없는 페이지에 접근했을 수 있습니다.</li>
		<br>
		<br>
			<c:choose>
				<c:when test="${empty session_MEM_ID}">
					<li> <p style="color:gray;">상태 : 로그인 되지 않았습니다. 로그인을 해주세요.</p></li></c:when>
	...
	<c:if test="${not empty session_MEM_ID}">
	...
	<c:if test="${empty session_MEM_ID}">
		<button class="btn btn-outline-dark" id="main" name="main" style="" onclick="location.href = '../member/loginForm'">로그인</button>
	</c:if>
	<br/> <br/> <br/> <br/> <br/> 
